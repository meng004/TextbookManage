...
namespace TextbookManage.Services.Jw
{
    ...
    public class TeacherService : ITeacherService
    {
        ...
        private readonly ITeacherService _serviceImpl = ServiceLocator.Current.GetInstance<ITeacherService>();


        public IEnumerable<Teacher> GetByDepartmentId(string departmentId)
        {
            return _serviceImpl.GetByDepartmentId(departmentId);
        }

        public Teacher GetById(string teacherId)
        {
            return _serviceImpl.GetById(teacherId);
        }
    }
}
